...
export function RegistryDetailPanel({
  ...
  const fetchInstance = useCallback(async () => {
    ...
    try {
      ...
      setError('Erro ao carregar dados de aprovaÃ§Ã£o.');
      setInstance(null);
    } finally {
      setLoading(false);
    }
  ...
  const shortHash = contentHash.length > 8 ? `${contentHash.slice(0, 8)}â€¦` : contentHash;
  ...
  const handleSubmitForReview = async () => {
    ...
    try {
      ...
      setSubmitError('Erro ao submeter para revisÃ£o.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleCancelInstance = async () => {
    const reason = window.prompt('Motivo do cancelamento da instÃ¢ncia:');
    ...
    try {
      ...
      setError('Erro ao cancelar instÃ¢ncia.');
    }
  };

  if (loading) {
    return <div className={styles.state}>Carregando painel de aprovaÃ§Ã£o...</div>;
  }
  ...
  return (
    <section className={styles.panel} aria-label="Painel de detalhes de aprovaÃ§Ã£o">
      ...
      {(effectiveFrom || effectiveTo) && (
        <div className={styles.section}>
          <strong>VigÃªncia:</strong> {formatDate(effectiveFrom)} â†’ {formatDate(effectiveTo)}
        </div>
      )}
